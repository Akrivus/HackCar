...
put '/' do
    if VECTOR_TYPES.include? params['vector']
        $tx = Thread.new { system("(while true; do sox './dir/#{params['vector']}.wav' -t wav -r 48000 -b 16 - repeat 30; done) | csdr convert_i16_f | csdr gain_ff #{ARGV[1]} | csdr convert_f_samplerf 20833 | sudo ../rpitx/rpitx -i - -m RF -f #{ARGV[0]}e3 -s 48000") }
    else
        $tx = nil
    end
end