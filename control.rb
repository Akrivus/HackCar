...
put '/' do
    if VECTOR_TYPES.include? params['vector']
        $tx = Thread.new { system("(while true; do sox './dir/#{params['vector']}.wav' -t wav -r 48k -b 16 - repeat 30; done) | csdr convert_i16_f | csdr gain_ff #{ARGV[1]} | csdr dsb_fc | sudo ../rpitx/rpitx -i - -m IQFLOAT -f #{ARGV[0]}e3 -s 48000") }
    else
        $tx = nil
    end
end